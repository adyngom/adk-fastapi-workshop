...
coordinator = LLMAgent(
    name="coordinator",
    tools=[travel_agent, news_agent, weather_agent]
)
```
...
# 🛠️ HANDS-ON: Your First Agent (15 minutes)

## Exercise Goal
Build a simple research assistant that can search the web and summarize findings

## Setup Steps

### 1. Install ADK (5 minutes)
```bash
...
research_agent = LLMAgent(
    ...
    description="Searches the web and provides concise summaries",
    model="gemini-2.0-flash",
...
writer = LLMAgent(
    ...
    instructions="Use research from {research_findings}",
...
editor = LLMAgent(
    ...
    instructions="Edit this draft: {draft_content}",
...
fact_checker = LLMAgent(
    ...
    instructions="Verify facts in: {edited_content}",
...
competitor_a_researcher = LLMAgent(
...
competitor_b_researcher = LLMAgent(
...
competitor_c_researcher = LLMAgent(
...
news_gatherer = LLMAgent(
    ...
    tools=[GoogleSearchTool()],
    instructions="Search for recent news about: {topic}",
...
summarizer = LLMAgent(
    ...
    instructions="Summarize these articles: {news_articles}",
    output_key="summary"
)
...
sentiment_analyzer = LLMAgent(
    ...
    instructions="Analyze sentiment in: {summary}",
    output_key="analysis"
)
```
...
**Method 1: Placeholder syntax in instructions**
```python
agent2 = LLMAgent(
    instructions="Book hotels near these attractions: {attractions_list}"
)
```
...
```python
from adk.eval import BaseEvaluator
